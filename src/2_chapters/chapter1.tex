...
In the same way mathematics as a whole has been summarized as three simple tasks—\textit{count}, \textit{measure}, and \textit{sort}—we could group statistical problems in three main groups. The problem of \textit{sampling}—selecting a finite group of observations from a larger, unknown population— the \textit{estimation}—build some mathematical quantity that represents how the measurements of my sample are distributed, and finally \textit{visualization}—how my observations look like, and how that changes if I represent them in one way or another. Again, all of these problems are related to the phenomenon of \textit{uncertainty}, or \textit{variation} among measurements.
...
\end{equation}
where the elements the sample $x_i$ consist of just a selected group of observations from the population, not necessarily consecutive or in the same order. The population represents the ideal object of inference, while the sample is the concrete, finite evidence available to us.  As an example, if I want to study some disease and its relation smokers in a given country, I will never have access to the \textit{complete population}, but only the amout of them that I am able to question, measure, or survey. This distinction is far from trivial. A poorly chosen sample often misrepresents the population and may induce bias, whereas a carefully constructed one mirrors its essential features, and can be used to describe the underlying nature.
...
Once we have a clear distinction between the population under study and the selected sample, we face a problem. Neither the population—referred as the \textit{true}—mean value, sometimes written as $\mu$, nor its variance–referred as the \textit{true} variance, and written as $\sigma^2$ are available to us. As we just saw, the \textit{only thing we have is the finite set of observations in our sample}, hence we could try to build some "informative quantities" out of out data that would give us a hint of the central value, a measure of spread, etc. Such quantities are called \textit{statistical estimators}. Common examples of such estimators are the \textit{sample mean}, the \textit{median}, and the \textit{variance}, among others.
...
As we see, the sample mean captures information about the "central" value, where most events cluster. Although useful, it is sensitive to extreme values—often called \textit{outliers}—which motivates the definition additional, more robust measures of central tendency.
...
A straightforward measure ofter used is the \textit{mode}, the value—or values—that appear most frequently in the observation set. For the first sample $\mathcal{S} = \{1, 2, 3, 5, 3, 2, 7\}$ we just count the frequency of each value, and conclude that since both $2$ and $3$ occur most frequently, the dataset is \textit{bimodal}, with modes $2$ and $3$. In the case of categorical data, such as eye colour or smartphone brands, the mode corresponds to the most common category.

\medskip

Beyond central location, it is important to understand the \textit{spread} of the data. We can define the \textit{sample variance} $s^2$ of a set as a quantity that captures how far are the elements from the mean value,
\begin{equation}
	s^2 = \frac{1}{n - 1} \sum_{i = 1}^{N} (x_{i} - \bar{x})^{2} \; ,
	\label{eq:sample_variance}. 
\end{equation}

The $n-1$ in the denominator of \eqref{eq:sample_variance} is called the Bessel correction factor, which ensures that only out of at leas $n = 2$ elements we can compute a finite variance. A more techical explanation is that it ensures that $s^2$ is an \textit{unbiased estimator}, which we will discuss in Chapter \ref{chapter3}
...
\begin{figure}[ht]
    ...
    \begin{subfigure}{0.8\textwidth}
        ...
        \caption{Three sets of observations, with the mean value and standard deviation represented as a histogram-based plot [...].}
    ...
    \begin{subfigure}{0.8\textwidth}
        ...
        \caption{Three sets of observations, with the mean value and standard deviation represented as a box plot [...].}
    ...
    \begin{subfigure}{0.8\textwidth}
        ...
        \caption{Three sets of observations, with the mean value and standard deviation represented as a violin plot [...].}
        \label{fig:mean_std_violin}
    \end{subfigure}

    % ----------- Global caption -----------
    \caption{Comparison of three visualization methods—histogram, box plot, and violin plot—showing the mean and variability of three samples of size $n = 100$.}
